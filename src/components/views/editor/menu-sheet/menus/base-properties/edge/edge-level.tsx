...
export function EdgeLevel({
    ...
    return (
        <Fragment key={levelKey}>
            <div className="flex flex-col p-1 ">
                <div className="flex w-full flex-row justify-end">
                    <Button
                        ...
                        className={twJoin(
                            'hidden flex-row',
                            (edge.data?.levels.length ?? 1) > 1 && 'flex'
                        )}
                ...
                <FieldGroup
                    className={twJoin(
                        'flex flex-col items-center gap-2 pl-3 pb-2 pr-6',
                        edge.data?.levels.length === 1 && 'pr-3 pt-3'
                    )}
                >
                    <form.AppField
                        name="target"
                        children={(field) => (
                            <field.NumberField
                                ...
                                incrementTooltip={
                                    isAtMaxTargetLevel ? (
                                        <p>
                                            Target level can't be greater than
                                            the source node activity levels
                                        </p>
                                    ) : undefined
                                }
                            />
                    ...
                    <form.AppField
                        ...
                        children={(field) => (
                            <ToggleGroup
                                className="flex w-full min-w-0 flex-row justify-center"
                                variant="outline"
                                type="single"
                                ...
                                onValueChange={(type) => {
                                    if (!type) return
                                    field.handleChange(type as InteractionType)
                                }}
                            >
                                <ToggleGroupItem
                                    className="min-w-0 flex-1 data-[state=on]:bg-transparent"
                                    value="activation"
            ...
            <Separator
                hidden={levelKey === (edge.data?.levels.length ?? 1) - 1}
                className="my-1"
            />
        </Fragment>
    )
